...
import com.klui.R;
import com.klui.refresh.api.DefaultRefreshFooterCreator;
import com.klui.refresh.api.DefaultRefreshHeaderCreator;
import com.klui.refresh.api.RefreshContent;
import com.klui.refresh.api.RefreshFooter;
import com.klui.refresh.api.RefreshHeader;
import com.klui.refresh.api.RefreshInternal;
import com.klui.refresh.api.RefreshKernel;
import com.klui.refresh.api.RefreshLayout;
import com.klui.refresh.api.ScrollBoundaryDecider;
import com.klui.refresh.constant.DimensionStatus;
...
import com.klui.refresh.header.BezierRadarHeader;
...
import com.klui.refresh.listener.OnMultiPurposeListener;
import com.klui.refresh.listener.OnRefreshListener;
import com.klui.refresh.listener.OnRefreshLoadMoreListener;
import com.klui.refresh.listener.OnStateChangedListener;
import com.klui.refresh.util.DelayedRunnable;
import com.klui.refresh.util.DensityUtil;
import com.klui.refresh.util.ScrollBoundaryUtil;
import com.klui.refresh.util.ViscousFluidInterpolator;
...
import static com.klui.refresh.util.DensityUtil.dp2px;
import static com.klui.refresh.util.SmartUtil.fling;
import static com.klui.refresh.util.SmartUtil.getColor;
import static com.klui.refresh.util.SmartUtil.isScrollableView;
...
public class SmartRefreshLayout extends ViewGroup
    ...
    public SmartRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDisableContentWhenRefresh =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlDisableContentWhenLoading, mDisableContentWhenLoading);
        mEnableHeaderTranslationContent =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableHeaderTranslationContent,
                        mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableFooterTranslationContent,
                        mEnableFooterTranslationContent);
        mEnablePreviewInEditMode =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnablePreviewInEditMode, mEnablePreviewInEditMode);
        ...
        mEnableOverScrollBounce =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableOverScrollBounce, mEnableOverScrollBounce);
        mEnablePureScrollMode =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableScrollContentWhenLoaded,
                mEnableScrollContentWhenLoaded);
        mEnableScrollContentWhenRefreshed =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableScrollContentWhenRefreshed,
                        mEnableScrollContentWhenRefreshed);
        mEnableLoadMoreWhenContentNotFull =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableLoadMoreWhenContentNotFull,
                        mEnableLoadMoreWhenContentNotFull);
        mEnableFooterFollowWhenLoadFinished =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableFooterFollowWhenLoadFinished,
                        mEnableFooterFollowWhenLoadFinished);
        mEnableClipHeaderWhenFixedBehind =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableClipHeaderWhenFixedBehind,
                        mEnableClipHeaderWhenFixedBehind);
        mEnableClipFooterWhenFixedBehind =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableClipFooterWhenFixedBehind,
                        mEnableClipFooterWhenFixedBehind);
        mEnableOverScrollDrag =
                ta.getBoolean(R.styleable.SmartRefreshLayout_srlEnableOverScrollDrag, mEnableOverScrollDrag);
        ...
        mHeaderTranslationViewId =
                ta.getResourceId(R.styleable.SmartRefreshLayout_srlHeaderTranslationViewId, View.NO_ID);
        mFooterTranslationViewId =
                ta.getResourceId(R.styleable.SmartRefreshLayout_srlFooterTranslationViewId, View.NO_ID);
        ...
        mHeaderHeightStatus =
                ta.hasValue(R.styleable.SmartRefreshLayout_srlHeaderHeight) ? DimensionStatus.XmlLayoutUnNotify
                        : mHeaderHeightStatus;
        mFooterHeightStatus =
                ta.hasValue(R.styleable.SmartRefreshLayout_srlFooterHeight) ? DimensionStatus.XmlLayoutUnNotify
                        : mFooterHeightStatus;
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < count; i++) {
            View view = super.getChildAt(i);
            if (i == indexHeader || (i != indexFooter
                    && indexHeader == -1
                    && mRefreshHeader == null
                    && view instanceof RefreshHeader)) {
                mRefreshHeader =
                        (view instanceof RefreshHeader) ? (RefreshHeader) view : new RefreshHeaderWrapper(view);
            } else if (i == indexFooter || (indexFooter == -1 && view instanceof RefreshFooter)) {
                mEnableLoadMore = (mEnableLoadMore || !mManualLoadMore);
                mRefreshFooter =
                        (view instanceof RefreshFooter) ? (RefreshFooter) view : new RefreshFooterWrapper(view);
                    ...
                    public void run() {
                        ...
                        for (ViewParent parent = thisView.getParent(); parent != null; ) {
                            if (parent instanceof NestedScrollingParent) {
                                ...
                                if (((NestedScrollingParent) parent).onStartNestedScroll(target, target,
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                if (mHeaderHeightStatus.gteReplaceWith(DimensionStatus.XmlLayoutUnNotify)) {
                    ...
                    if (!mHeaderHeightStatus.notified) {
                        super.measureChild(headerView, widthSpec, makeMeasureSpec(
                                Math.max(getSize(heightSpec) - lp.bottomMargin - lp.topMargin, 0), AT_MOST));
                        headerHeight = headerView.getMeasuredHeight();
                    }
                    headerView.measure(widthSpec, makeMeasureSpec(
                            Math.max(getSize(heightSpec) - lp.bottomMargin - lp.topMargin, 0), EXACTLY));
                    ...
                    heightSpec =
                            makeMeasureSpec(Math.max(getSize(heightMeasureSpec) - lp.bottomMargin - lp.topMargin, 0),
                                    AT_MOST);
                    headerView.measure(widthSpec, heightSpec);
                    ...
                    if (measuredHeight > 0 && mHeaderHeightStatus.canReplaceWith(DimensionStatus.XmlWrapUnNotify)) {
                        ...
                        heightSpec =
                                makeMeasureSpec(Math.max(mHeaderHeight - lp.bottomMargin - lp.topMargin, 0), EXACTLY);
                        headerView.measure(widthSpec, heightSpec);
                    }
            ...
            if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
                ...
                if (mFooterHeightStatus.gteReplaceWith(DimensionStatus.XmlLayoutUnNotify)) {
                    ...
                    if (!mFooterHeightStatus.notified) {
                        super.measureChild(footerView, widthSpec,
                                makeMeasureSpec(getSize(heightSpec) - lp.topMargin - lp.bottomMargin, AT_MOST));
                        footerHeight = footerView.getMeasuredHeight();
                    }
                    footerView.measure(widthSpec,
                            makeMeasureSpec(getSize(heightSpec) - lp.topMargin - lp.bottomMargin, EXACTLY));
                    ...
                    heightSpec =
                            makeMeasureSpec(Math.max(getSize(heightMeasureSpec) - lp.topMargin - lp.bottomMargin, 0),
                                    AT_MOST);
                    footerView.measure(widthSpec, heightSpec);
                    ...
                    if (measuredHeight > 0 && mFooterHeightStatus.canReplaceWith(DimensionStatus.XmlWrapUnNotify)) {
                        ...
                        heightSpec =
                                makeMeasureSpec(Math.max(mFooterHeight - lp.topMargin - lp.bottomMargin, 0), EXACTLY);
                        footerView.measure(widthSpec, heightSpec);
                    }
                } else if (lp.height == MATCH_PARENT) {
                    heightSpec = makeMeasureSpec(Math.max(mFooterHeight - lp.topMargin - lp.bottomMargin, 0), EXACTLY);
            ...
            if (mRefreshContent != null && mRefreshContent.getView() == child) {
                ...
                final int heightSpec = ViewGroup.getChildMeasureSpec(heightMeasureSpec,
                        thisView.getPaddingTop() + thisView.getPaddingBottom() +
                                ...
                                ((isInEditMode && isEnableRefresh() && mRefreshHeader != null && (
                                        mEnableHeaderTranslationContent
                                                || mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind))
                                        ? mHeaderHeight : 0) +
                                ((isInEditMode && isEnableLoadMore() && mRefreshFooter != null && (
                                        mEnableFooterTranslationContent
                                                || mRefreshFooter.getSpinnerStyle() == SpinnerStyle.FixedBehind))
                                        ? mFooterHeight : 0), lp.height);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (mRefreshContent != null && mRefreshContent.getView() == child) {
                boolean isPreviewMode = thisView.isInEditMode()
                        && mEnablePreviewInEditMode
                        && isEnableRefresh()
                        && mRefreshHeader != null;
                final View contentView = mRefreshContent.getView();
                ...
                if (isPreviewMode && (mEnableHeaderTranslationContent
                        || mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind)) {
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
            ...
            if (contentView != null) {
                int top = Math.min(contentView.getBottom() - contentView.getPaddingBottom() + mSpinner,
                        child.getBottom());
            ...
            if ((finalY < 0 && (mEnableOverScrollDrag || isEnableRefresh()) && mRefreshContent.canRefresh())
                ...
                if (mVerticalPermit) {
                    ...
                    if (Build.VERSION.SDK_INT >= 14) {
                        ...
                        velocity = 1f * (mScroller.getCurrY() - finalY) / Math.max(
                                (mScroller.getDuration() - mScroller.timePassed()), 1);
                    }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        if (mNestedInProgress) {//嵌套滚动时，补充竖直方向不滚动，但是水平方向滚动，需要通知 onHorizontalDrag
            ...
            return ret;
                ...
                || (mHeaderNeedTouchEventWhenRefreshing && ((mState.isOpening || mState.isFinishing)
                && mState.isHeader))
                || (mFooterNeedTouchEventWhenLoading && ((mState.isOpening || mState.isFinishing)
                && mState.isFooter))) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mIsBeingDragged
                    ...
                    if (mDragDirection == 'v' || (Math.abs(dy) >= mTouchSlop && Math.abs(dx) < Math.abs(
                            dy))) {//滑动允许最大角度为45度
                        mDragDirection = 'v';
                        if (dy > 0 && (mSpinner < 0 || ((mEnableOverScrollDrag || isEnableRefresh())
                                && mRefreshContent.canRefresh()))) {
                            mIsBeingDragged = true;
                            mTouchY = touchY - mTouchSlop;//调整 mTouchSlop 偏差
                        } else if (dy < 0 && (mSpinner > 0 || ((mEnableOverScrollDrag || isEnableLoadMore()) && ((mState
                                == RefreshState.Loading && mFooterLocked) || mRefreshContent.canLoadMore())))) {
                            mIsBeingDragged = true;
                            mTouchY = touchY + mTouchSlop;//调整 mTouchSlop 偏差
                        }
                        if (mIsBeingDragged) {
                ...
                if (mIsBeingDragged) {
                    ...
                    if ((mViceState.isHeader && (spinner < 0 || mLastSpinner < 0)) || (mViceState.isFooter && (spinner
                            > 0 || mLastSpinner > 0))) {
                        ...
                        MotionEvent em =
                                obtain(time, time, MotionEvent.ACTION_MOVE, mTouchX + dx, mTouchY + spinner, 0);
                        super.dispatchTouchEvent(em);
                        if (mFooterLocked && dy > mTouchSlop && mSpinner < 0) {
                            mFooterLocked = false;//内容向下滚动时 解锁Footer 的锁定
                        }
                        if (spinner > 0 && ((mEnableOverScrollDrag || isEnableRefresh())
                                && mRefreshContent.canRefresh())) {
                            mTouchY = mLastTouchY = touchY;
                            mTouchSpinner = spinner = 0;
                            mKernel.setState(RefreshState.PullDownToRefresh);
                        } else if (spinner < 0 && ((mEnableOverScrollDrag || isEnableLoadMore())
    ...
    protected boolean startFlingIfNeed(Float flingVelocity) {
        ...
        if (Math.abs(velocity) > mMinimumVelocity) {
            if (velocity * mSpinner < 0) {
                ...
                } else if (mSpinner > mHeaderHeight * mHeaderTriggerRate
                        || -mSpinner > mFooterHeight * mFooterTriggerRate) {
                    return true;//拦截嵌套滚动时，即将刷新或者加载的 Fling
                }
            }
            if ((velocity < 0 && ((mEnableOverScrollBounce && (mEnableOverScrollDrag || isEnableLoadMore())) || (mState
                    == RefreshState.Loading && mSpinner >= 0) || (mEnableAutoLoadMore && isEnableLoadMore())))
    ...
    protected void setStateDirectLoading() {
        if (mState != RefreshState.Loading) {
            ...
            if (mOnMultiPurposeListener != null && mRefreshFooter instanceof RefreshFooter) {
                ...
                mOnMultiPurposeListener.onFooterStartAnimator((RefreshFooter) mRefreshFooter, mFooterHeight,
    ...
    protected void setStateLoading() {
        ...
        if (mOnMultiPurposeListener != null && mRefreshFooter instanceof RefreshFooter) {
            ...
            mOnMultiPurposeListener.onFooterReleased((RefreshFooter) mRefreshFooter, mFooterHeight,
                    (int) (mFooterMaxDragRate * mFooterHeight));
        }
        if (animator == null) {
    ...
    protected void setStateRefreshing() {
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mRefreshHeader != null) {
                    mRefreshHeader.onStartAnimator(SmartRefreshLayout.this, mHeaderHeight,
                ...
                if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    ...
                    mOnMultiPurposeListener.onHeaderStartAnimator((RefreshHeader) mRefreshHeader, mHeaderHeight,
                            (int) (mHeaderMaxDragRate * mHeaderHeight));
                }
            }
        };
        ...
        if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
            ...
            mOnMultiPurposeListener.onHeaderReleased((RefreshHeader) mRefreshHeader, mHeaderHeight,
                    (int) (mHeaderMaxDragRate * mHeaderHeight));
        }
        if (animator == null) {
    ...
    protected class FlingRunnable implements Runnable {
        ...
        public Runnable start() {
            ...
            if (mSpinner != 0 && (!(mState.isOpening || (mFooterNoMoreData
                    && mEnableFooterFollowWhenLoadFinished
                    && isEnableLoadMore()))
                    || ((mState == RefreshState.Loading || (mFooterNoMoreData
                    && mEnableFooterFollowWhenLoadFinished
                    && isEnableLoadMore())) && mSpinner < -mFooterHeight)
        ...
        public void run() {
            if (animationRunnable == this && !mState.isFinishing) {
                ...
                if (Math.abs(velocity) >= 1) {
                    ...
                    animationRunnable = null;
                    if (Math.abs(mSpinner) >= Math.abs(mSmoothDistance)) {
                        int duration = 10 * Math.min(
                                Math.max((int) DensityUtil.px2dp(Math.abs(mSpinner - mSmoothDistance)), 30), 100);
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            ...
            backgroundColor =
                    ta.getColor(R.styleable.SmartRefreshLayout_Layout_layout_srlBackgroundColor, backgroundColor);
            if (ta.hasValue(R.styleable.SmartRefreshLayout_Layout_layout_srlSpinnerStyle)) {
                spinnerStyle =
                        SpinnerStyle.values()[ta.getInt(R.styleable.SmartRefreshLayout_Layout_layout_srlSpinnerStyle,
    ...
    public boolean onStartNestedScroll(@NonNull View child, @NonNull View target, int nestedScrollAxes) {
        final View thisView = this;
        boolean accepted = thisView.isEnabled()
     ...
     * 设置多功能监听器
     *
     * @param listener 建议使用 {@link com.klui.refresh.listener.SimpleMultiPurposeListener}
     * @return SmartRefreshLayout
     */
    @Override
     ...
     * 设置滚动边界
     *
     * @param boundary 建议使用 {@link com.klui.refresh.impl.ScrollBoundaryDeciderAdapter}
     * @return SmartRefreshLayout
     */
    @Override
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            SmartRefreshLayout.super.dispatchTouchEvent(
                                    obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX,
                                            mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                            SmartRefreshLayout.super.dispatchTouchEvent(
                                    obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner,
                                            0));
                        ...
                        if (mSpinner > 0) {
                            ...
                            ValueAnimator
                                    valueAnimator = animSpinner(0, startDelay, mReboundInterpolator, mReboundDuration);
        ...
        postDelayed(new Runnable() {
            ...
            public void run() {
                if (mState == RefreshState.Loading && mRefreshFooter != null && mRefreshContent != null) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        //计算布局将要移动的偏移量
                        final boolean needHoldFooter = noMoreData
                                && mEnableFooterFollowWhenLoadFinished
                                && mSpinner < 0
                                && mRefreshContent.canLoadMore();
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            SmartRefreshLayout.super.dispatchTouchEvent(
                                    obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX,
                                            mLastTouchY + offset + mTouchSlop * 2, 0));
                            SmartRefreshLayout.super.dispatchTouchEvent(
                                    obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offset, 0));
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                            }
    ...
    public boolean autoRefresh() {
        return autoRefresh(mHandler == null ? 400 : 0, mReboundDuration,
                1f * ((mHeaderMaxDragRate / 2 + 0.5f) * mHeaderHeight) / (mHeaderHeight == 0 ? 1 : mHeaderHeight));
    }
    ...
    public boolean autoLoadMore() {
        return autoLoadMore(0, mReboundDuration,
                1f * (mFooterHeight * (mFooterMaxDragRate / 2 + 0.5f)) / (mFooterHeight == 0 ? 1 : mFooterHeight));
    }
    ...
    public class RefreshKernelImpl implements RefreshKernel {
        ...
        public RefreshKernel setState(@NonNull RefreshState state) {
            switch (state) {
                ...
                case PullDownToRefresh:
                    ...
                    break;
                case PullUpToLoad:
                    if (isEnableLoadMore() && !mState.isOpening && !mState.isFinishing && !(mFooterNoMoreData
                            && mEnableFooterFollowWhenLoadFinished)) {
                ...
                case PullDownCanceled:
                    ...
                    break;
                case PullUpCanceled:
                    if (isEnableLoadMore() && !mState.isOpening && !(mFooterNoMoreData
                            && mEnableFooterFollowWhenLoadFinished)) {
        ...
        public RefreshKernel moveSpinner(int spinner, boolean isDragging) {
            ...
            if (mRefreshContent != null) {
                ...
                if (spinner >= 0 && mRefreshHeader != null) {
                    if (mEnableHeaderTranslationContent
                            || mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
                        tSpinner = spinner;
                ...
                if (spinner <= 0 && mRefreshFooter != null) {
                    if (mEnableFooterTranslationContent
                            || mRefreshFooter.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
                        tSpinner = spinner;
                    } else if (oldSpinner > 0) {
                        tSpinner = 0;
                ...
                if (tSpinner != null) {
                    ...
                    boolean header = mEnableClipHeaderWhenFixedBehind
                            && mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind;
                    header = header || mHeaderBackgroundColor != 0;
                    boolean footer = mEnableClipFooterWhenFixedBehind
                            && mRefreshFooter.getSpinnerStyle() == SpinnerStyle.FixedBehind;
                    footer = footer || mFooterBackgroundColor != 0;
                    if ((header && (tSpinner >= 0 || oldSpinner > 0)) || (footer && (tSpinner <= 0
                            || oldSpinner < 0))) {
                        thisView.invalidate();
                    }
                }
            ...
            if ((spinner >= 0 || oldSpinner > 0) && mRefreshHeader != null) {
                ...
                if (oldSpinner != mSpinner
                    ...
                    mOnMultiPurposeListener.onHeaderMoving((RefreshHeader) mRefreshHeader, isDragging, percent, offset,
            ...
            if ((spinner <= 0 || oldSpinner < 0) && mRefreshFooter != null) {
                ...
                if (oldSpinner != mSpinner
                    ...
                    mOnMultiPurposeListener.onFooterMoving((RefreshFooter) mRefreshFooter, isDragging, percent, offset,
        ...
        @Override
        public RefreshKernel requestDefaultTranslationContentFor(@NonNull RefreshInternal internal,
                boolean translation) {
            if (mRefreshHeader != null && mRefreshHeader.getView() == internal.getView()) {
    ...
    public boolean post(@NonNull Runnable action) {
        if (mHandler == null) {
            mListDelayedRunnable =
                    mListDelayedRunnable == null ? new ArrayList<DelayedRunnable>() : mListDelayedRunnable;
